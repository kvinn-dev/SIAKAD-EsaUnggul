...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'ringkasan' | 'mata_kuliah' | 'absensi' | 'nilai' | 'pembayaran' | 'notifikasi' | 'profil' | 'performa'>('ringkasan');
        ...
        fetch('/api/notifications', { headers })
          ...
          .then(res => {
            if (res.success && res.data) {
              ...
              const mailLogs = res.data.filter((n: any) => n.type === 'sistem' || n.title.toLowerCase().includes('otp') || n.title.toLowerCase().includes('sandi'));
  ...
  const fetchStudentData = async (activeToken: string) => {
    ...
    addEsbLog(`Fetching Student bundle through ${useWso2Gateway ? 'WSO2 Carbon Gateway' : 'Direct API'}`);

    try {
      // 1. Fetch Summary
      const sumRes = await fetch(`${apiPrefix}/dashboard/summary`, { headers: authHeaders });
      ...
      if (sumData.success) {
        ...
        if (useWso2Gateway) {
          ...
          addEsbLog(`WSO2 Proxy OK: ${sumRes.status} | Gateway Overhead: ${gwMs}ms | Execution: ${totalMs}ms`);
        }
      }

      // 2. Fetch Courses
      const crsRes = await fetch(`${apiPrefix}/courses`, { headers: authHeaders });
      const crsData = await crsRes.json();
      if (crsData.success) setCourses(crsData.data);

      // 3. Fetch Schedules
      const schRes = await fetch(`${apiPrefix}/schedules`, { headers: authHeaders });
      const schData = await schRes.json();
      if (schData.success) setSchedules(schData.data);

      // 4. Fetch Attendances
      const attRes = await fetch(`${apiPrefix}/attendance`, { headers: authHeaders });
      const attData = await attRes.json();
      if (attData.success) setAttendance(attData.data);

      // 5. Fetch Grades
      const grdRes = await fetch(`${apiPrefix}/grades`, { headers: authHeaders });
      const grdData = await grdRes.json();
      if (grdData.success) setGrades(grdData.data);

      // 6. Fetch Payments
      const payRes = await fetch(`${apiPrefix}/payments`, { headers: authHeaders });
      const payData = await payRes.json();
      if (payData.success) setPayments(payData.data);

      // 7. Fetch Notifications
      const notRes = await fetch(`${apiPrefix}/notifications`, { headers: authHeaders });
      ...
      console.error(err);
      triggerAlert('error', 'Gagal memuat data akademik mahasiswa. Periksa koneksi backend.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!loginForm.username || !loginForm.password) {
      return triggerAlert('error', 'Silakan masukkan NIM atau Email dan Kata Sandi.');
    }
    setIsLoading(true);
    ...
    try {
      ...
      if (data.success) {
        if (data.requires_otp) {
          ...
          addEsbLog(`Auth success. MFA OTP triggered. Simulating dispatch email message to ${data.email}.`);
        } else {
          setToken(data.token);
          setUser(data.user);
          setPage('dashboard');
          triggerAlert('success', data.message);
          addEsbLog(`Access granted for ${data.user.name}. Session Token Generated.`);
        }
      } else {
        triggerAlert('error', data.message || 'Kombinasi NIM/Email/Sandi salah.');
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPage('otp');
        triggerAlert('success', data.message);
        addEsbLog(`Account created. Dispatched verification mail to: ${data.email}`);
      } else {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      if (otpPurpose === 'forgot') {
        ...
        triggerAlert('success', 'OTP valid. Silakan buat kata sandi baru Anda.');
        setIsLoading(false);
        return;
      }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        triggerAlert('success', data.message || 'OTP baru telah dikirim kembali ke email.');
  ...
  const handleForgotEmail = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!forgotEmail) return triggerAlert('error', 'Tulis email terdaftar Anda.');
    setIsLoading(true);
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setPage('login');
        triggerAlert('success', data.message);
        addEsbLog(`Credentials reset completely. Redirecting user to login panel.`);
      } else {
        triggerAlert('error', data.message);
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    addEsbLog(`Pushing student security password update on current active profile.`);
  ...
  const handleSimulatePayment = async (paymentId: string) => {
    setIsLoading(true);
    addEsbLog(`Initiating Student Billing payment simulation for: ${paymentId}`);
    ...
    try {
      const response = await fetch('/api/payments/upload-proof', {
        ...
        body: JSON.stringify({
          paymentId,
          proofUrl: 'https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?auto=format&fit=crop&q=80&w=400'
        })
      });
      const data = await response.json();
      if (data.success) {
        triggerAlert('success', 'Simulasi Pembayaran Berhasil! Bukti transfer dikirim ke keuangan.');
        addEsbLog(`MOCK PAYMENT: Uploaded transactional receipt for billing invoice id: ${paymentId}.`);
        fetchStudentData(token!);
      } else {
        triggerAlert('error', data.message);
      }
  ...
  const handleTriggerBenchmark = async (type: 'REST' | 'SOAP') => {
    ...
    try {
      ...
      if (data.success) {
        ...
        addEsbLog(`Bench Completed: ${type} took ${data.timeMs}ms | Payload: ${data.payloadSize} Bytes | Type: ${data.payloadFormat}`);
        fetchPerformanceLogs();
      }
    } catch (e) {
  ...
  const getAutoConclusion = () => {
    ...
    const avgRestTime = Math.round(rLogs.reduce((acc, l) => acc + l.time, 0) / rLogs.length * 100) / 100;
    const avgSoapTime = Math.round(sLogs.reduce((acc, l) => acc + l.time, 0) / sLogs.length * 100) / 100;

    const avgRestSize = Math.round(rLogs.reduce((acc, l) => acc + l.size, 0) / rLogs.length);
    const avgSoapSize = Math.round(sLogs.reduce((acc, l) => acc + l.size, 0) / sLogs.length);
...
WSO2 ESB bertindak sebagai middleware integrasi yang tangguh dengan memproses transformasi routing dan menyuntikkan Carbon metadata header. Untuk sistem mahasiswa berskala mikro, REST JSON merupakan pilihan ideal karena ringan, mudah dibaca, dan berkecepatan tinggi. Namun, untuk integrasi level enterprise antar sistem perbankan/akreditasi kampus yang memerlukan kontrak layanan ketat, SOAP API dengan jaminan WSDL XML schema memberikan keamanan transaksi yang lebih formal dan handal.`;
  ...
  return (
    <div id="siakad-app-container" className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col selection:bg-sky-200">

      {/* Alert Top Banner */}
      {alert && (
        <div
          id="global-alert-toast"
          className={`fixed top-[62px] sm:top-[74px] left-1/2 -translate-x-1/2 z-50 flex items-center justify-between gap-3 px-3 py-1.5 rounded-full border shadow-md text-xs max-w-[92vw] sm:max-w-md transition-all duration-300 animate-in fade-in slide-in-from-top-1 ${alert.type === 'success'
        ...
        >
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium tracking-tight whitespace-nowrap overflow-hidden text-ellipsis">{alert.message}</span>
          </div>
          <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 relative z-10">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="space-y-0.5">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold tracking-tight flex items-center gap-2 flex-wrap">
                <span className="font-display">SIAKAD</span>
                <span className="text-sky-100 font-medium text-xs sm:text-sm bg-white/10 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg border border-white/10 font-sans">
                  Esa Unggul
                </span>
              </h1>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => {
                ...
                if (nextVal) {
                  triggerAlert('success', 'Mode Developer Aktif! Menu "Integrasi & Performa" kini tersedia di sidebar.');
                } else {
                  triggerAlert('success', 'Mode Developer Dinonaktifkan.');
                  if (activeTab === 'performa') setActiveTab('ringkasan');
                }
              }}
              className={`p-2 rounded-lg transition-all border shrink-0 ${showDevPanel
                ? 'bg-blueb-400 text-slate-900 border-blue-300 shadow-md hover:bg-blue-300'
                : 'bg-white/10 text-white/80 border-white/10 hover:bg-white/20 hover:text-white'
                }`}
              title={showDevPanel ? 'Sembunyikan Dev Tools' : 'Aktifkan Dev Tools'}
            >
            ...
            {user && (
              <div className="flex items-center gap-2 sm:gap-3 bg-white/10 backdrop-blur-sm border border-white/10 pl-3 sm:pl-4 pr-1.5 py-1.5 rounded-lg">
                ...
                <div className="hidden sm:block text-right leading-tight">
                  <p className="text-[11px] font-semibold text-white truncate max-w-[120px]">{user.name}</p>
                  <p className="text-[9px] text-sky-200/80 font-mono tracking-wide">{user.nim}</p>
                </div>
          ...
          {user ? (
            <div className="bg-white/80 backdrop-blur-md rounded-xl shadow-sm border border-slate-200/80 p-4 shrink-0">
              <div className="text-center pb-4 mb-4 border-b border-slate-100">
                <div className="relative w-20 h-20 mx-auto mb-3 rounded-full overflow-hidden border-2 border-sky-400 p-0.5 bg-sky-50 shadow-inner">
                  <img
                    src={profileForm.photo || "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?auto=format&fit=crop&q=80&w=200"}
                    alt="Foto Profil"
                ...
                <p className="text-[11px] text-slate-500 mt-1 mt-0.5">{dashboardData?.student?.study_program || 'Teknik Informatika'}</p>
              </div>
              ...
              <div className="space-y-1">
                {[
                  ...
                  { id: 'pembayaran', label: 'Keuangan & Tagihan', icon: CreditCard, count: payments.filter(p => p.status === 'Belum Lunas').length },
                  { id: 'notifikasi', label: 'Notifikasi Kampus', icon: Bell, count: notifications.filter(n => !n.is_read).length },
                  { id: 'profil', label: 'Profil Saya', icon: User },
                  ...(showDevPanel ? [{ id: 'performa', label: 'Integrasi & Performa', icon: BarChart3, highlight: true }] : [])
                ].map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <Icon className={`w-4 h-4 ${isActive ? 'text-white' : item.highlight ? 'text-amber-600' : 'text-slate-400'}`} />
                        <span className="font-display">{item.label}</span>
                      </div>
                      {item.count > 0 && !isActive && (
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${item.highlight || item.id === 'pembayaran' ? 'bg-rose-500 text-white' : 'bg-rose-100 text-rose-700'}`}>
                          {item.count}
                        </span>
                      )}
                    </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
              ...
              <p className="text-xs text-slate-500 leading-relaxed mb-3">
                Aplikasi ini mendukung perbandingan arsitektur SOA modern dengan menguji REST API (Format JSON yang ringan) dan SOAP Webservice (Format XML terproteksi WSDL kontrak) melalui WSO2 ESB.
              </p>
          ...
          {!user && (
            <div className="bg-gradient-to-br from-indigo-50 to-sky-50 rounded-xl border border-sky-100 p-4 shrink-0">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed mb-3">
                Klik tombol di bawah untuk langsung mengisi kolom input kredensial mahasiswa default Universitas Esa Unggul.
              </p>
              <button
                type="button"
                onClick={() => {
                  ...
                  triggerAlert('success', 'Kredensial mahasiswa berhasil disalin!');
                }}
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 grow">
          ...
          {!user && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8 grow flex flex-col justify-center">
              ...
              {page === 'login' && (
                <div id="auth-login-panel" className="max-w-md mx-auto w-full">
                  <div className="text-center mb-6">
                    <span className="text-[10px] font-semibold text-sky-600 tracking-widest uppercase bg-sky-50 px-2.5 py-1 rounded-full">Gate Portal Mahasiswa</span>
                    <h2 className="text-2xl font-bold text-slate-800 mt-2">Masuk SIAKAD</h2>
                    <p className="text-xs text-slate-500 mt-1">Gunakan alamat email Esa Unggul / NIM Anda</p>
                  </div>

                  <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-600 uppercase mb-1">NIM atau Email Kampus</label>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <label className="block text-[11px] font-bold text-slate-600 uppercase">Kata Sandi</label>
                        <button
                          type="button"
                  ...
                  <div className="text-center mt-6 pt-5 border-t border-slate-100 text-xs text-slate-500">
                    <p>Mahasiswa baru? <button onClick={() => setPage('register')} className="text-indigo-600 font-semibold hover:underline">Registrasi Akun di Sini</button></p>
                  </div>
                </div>
              )}
              ...
              {/* REGISTER PAGE */}
              {page === 'register' && (
                <div id="auth-register-panel" className="max-w-lg mx-auto w-full">
                  <div className="text-center mb-6">
                    <span className="text-[10px] font-bold text-sky-600 tracking-widest uppercase bg-sky-50 px-2.5 py-1 rounded-full font-mono">Daftar Akun Baru</span>
                    <h2 className="text-2xl font-bold text-slate-800 mt-2">Registrasi SIAKAD</h2>
                    <p className="text-xs text-slate-500 mt-1">Lengkapi dokumen integrasi kemahasiswaan Anda</p>
                  </div>
                  ...
                  <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">NIM Mahasiswa</label>
                      <input
                        type="text"
                        required
                        maxLength={11}
                        value={registerForm.nim}
                        onChange={(e) => setRegisterForm(prev => ({ ...prev, nim: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Email Kampus Gmail</label>
                      <input
                        type="email"
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Konfirmasi Sandi</label>
                      <input
                        type="password"
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Nomor Telepon</label>
                      <input
                        type="tel"
                        required
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Program Studi</label>
                      <select
                        value={registerForm.study_program}
                        onChange={(e) => setRegisterForm(prev => ({ ...prev, study_program: e.target.value }))}
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Fakultas Utama</label>
                      <input
                        type="text"
                        readOnly
                  ...
                  <div className="text-center mt-6 pt-5 border-t border-slate-100 text-xs text-slate-500">
                    <p>Sudah punya akun? <button onClick={() => setPage('login')} className="text-indigo-600 font-bold hover:underline">Masuk Ke Sini</button></p>
                  </div>
                </div>
              )}
              ...
              {page === 'otp' && (
                <div id="auth-otp-panel" className="max-w-md mx-auto w-full">
                  <div className="text-center mb-6">
                    ...
                    <span className="text-[10px] font-bold text-indigo-600 tracking-widest uppercase bg-indigo-50 px-2.5 py-1 rounded-full font-mono">Verifikasi Keamanan</span>
                    <h2 className="text-2xl font-bold text-slate-800 mt-2">Masukkan Kode OTP</h2>
                    <p className="text-xs text-slate-500 mt-2">
                      Kami mengirimkan kode 6-digit rahasia ke kotak masuk email: <span className="font-semibold text-slate-700">{otpEmail}</span>
                    </p>
                  </div>

                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    <div>
                      <label className="text-xs font-bold text-slate-600 uppercase block text-center mb-2">Kode OTP 6-Digit</label>
                      <input
                        id="otp-digits-input"
                        type="text"
                        maxLength={6}
                        required
                        value={otpForm}
                        onChange={(e) => setOtpForm(e.target.value.replace(/\D/g, ''))}
                        placeholder="000000"
              ...
              {page === 'forgot_password' && (
                <div id="auth-forgot-panel" className="max-w-md mx-auto w-full">
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-800">Ubah Kata Sandi</h2>
                    <p className="text-xs text-slate-500 mt-1">Masukkan kata sandi pengaman baru untuk akun SIAKAD Anda</p>
                  </div>
                  ...
                  {otpUserId ? (
                    ...
                    <form onSubmit={handleResetPassword} className="space-y-4">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Konfirmasi Kata Sandi Baru</label>
                        <input
                          type="password"
                          required
                    ...
                    <form onSubmit={handleForgotEmail} className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Alamat Email Terdaftar</label>
                        <input
                          type="email"
                          required
                          value={forgotEmail}
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="bg-gradient-to-br from-indigo-50 via-sky-50 to-white rounded-2xl shadow-sm border border-sky-100 p-5 relative overflow-hidden">
                ...
                <div className="relative flex flex-col md:flex-row items-center justify-between gap-5">
                  <div>
                    <span className="text-[10px] font-bold text-sky-600 tracking-wider bg-white border border-sky-100 px-2.5 py-1 rounded-full uppercase">Sistem Portal Mahasiswa</span>
                    <h2 className="text-xl md:text-2xl font-extrabold text-slate-800 mt-2">Selamat datang, {user.name}!</h2>
                    <p className="text-xs text-slate-500 mt-1 max-w-lg leading-relaxed">
                      SIAKAD Esa Unggul mengintegrasikan seluruh operasional perkuliahan Anda. Anda saat ini tercatat aktif di semester {dashboardData?.student?.semester || 4} pada Program Studi <span className="font-semibold text-slate-700">{dashboardData?.student?.study_program || 'Teknik Informatika'}</span>.
                    </p>
                  </div>

                  {/* Digital Live Clock */}
                  <div className="text-center py-2.5 px-4 bg-white rounded-xl shadow-xs border border-slate-100 shrink-0">
                    <span className="text-[10px] text-slate-400 block font-mono">WAKTU SERVER UTC</span>
                    <span className="text-sm font-bold text-indigo-600 font-mono tracking-wide">{new Date().toISOString().split('T')[1].slice(0, 8)}</span>
                    <p className="text-[9px] text-emerald-600 font-medium mt-0.5">● KONEKSI REST/SOAP AKTIF</p>
                  </div>
                </div>
              </div>
              ...
              {activeTab === 'ringkasan' && (
                <div id="tab-dashboard-ringkasan" className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {[
                      { title: 'Indeks Prestasi Kumulatif (IPK)', value: calculateGPAValue().toFixed(2), minTitle: 'Skala 4.00', color: 'border-sky-200 text-sky-700 bg-sky-50/50', icon: Award },
                      { title: 'Beban SKS Semester', value: String(dashboardData?.stats?.total_sks || 19), minTitle: 'Batas Maks 24', color: 'border-indigo-200 text-indigo-700 bg-indigo-50/50', icon: BookOpen },
                      { title: 'Mata Kuliah Aktif', value: String(dashboardData?.stats?.active_courses || courses.length), minTitle: 'Terdaftar KRS', color: 'border-emerald-200 text-emerald-700 bg-emerald-50/50', icon: Grid },
                      { title: 'Persentase Presensi', value: `${dashboardData?.stats?.attendance_percentage ?? 95}%`, minTitle: 'Syarat UAS 75%', color: 'border-teal-200 text-teal-700 bg-teal-50/50', icon: Activity },
                      { title: 'Tagihan Berjalan', value: String(dashboardData?.stats?.pending_bills || payments.filter(p => p.status === 'Belum Lunas').length), minTitle: 'Belum Dilunasi', color: 'border-rose-200 text-rose-700 bg-rose-50/50', icon: CreditCard },
                    ].map((stat: any, idx) => {
                      const StatIcon = stat.icon;
                      return (
                        <div key={idx} className={`p-4 rounded-xl border flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow ${stat.color}`}>
                          <div className="flex items-start justify-between gap-2 mb-3">
                            <span className="text-[10px] text-slate-500 font-bold uppercase leading-tight flex-1">{stat.title}</span>
                            <StatIcon className="w-4 h-4 shrink-0 opacity-70 mt-0.5" />
                          </div>
                          <div>
                            <span className="text-xl sm:text-2xl font-black block tracking-tight">{stat.value}</span>
                            <span className="text-[10px] text-slate-500/80 block mt-0.5 font-medium">{stat.minTitle}</span>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-5 xl:col-span-2">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-bold text-slate-800 text-sm">Grafik Indeks Prestasi (IP) per Semester</h3>
                        <span className="text-xs text-indigo-600 bg-indigo-50 font-bold px-2 py-0.5 rounded-lg border border-indigo-100">Evaluasi Studi</span>
                      </div>
                      ...
                      <div className="h-60 w-full text-xs">
                        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                          <AreaChart data={[
                            ...
                            { semester: 'Smt 4 (Kini)', IP: calculateGPAValue() }
                          ]}>
                            <defs>
                            ...
                            <YAxis domain={[0, 4]} ticks={[1, 2, 3, 4]} stroke="#94a3b8" />
                            <Tooltip />
                            <Area type="monotone" dataKey="IP" stroke="#6366f1" strokeWidth={2} fillOpacity={1} fill="url(#gpaGradient)" />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                      <div className="text-center mt-3 text-[10px] text-slate-400">
                        Catatan: IPK Kumulatif dihitung dinamais berdasarkan bobot sks mata kuliah terdaftar di SIAKAD.
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-5 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <h3 className="font-bold text-slate-800 text-sm">Kalender Kuliah Mingguan</h3>
                          <span className="text-[10px] text-sky-600 font-bold uppercase">Jadwal Aktif</span>
                        </div>
                        ...
                        <div className="space-y-2.5 max-h-[220px] overflow-y-auto">
                          {schedules.map((sch: any) => {
                            ...
                            return (
                              <div
                                key={sch.id}
                                className={`p-2.5 rounded-xl border text-xs leading-normal transition-all ${isToday
                                  ? 'bg-sky-50 border-sky-300 ring-1 ring-sky-300'
                                  : 'bg-slate-50/50 border-slate-100 hover:bg-slate-50'
                                  }`}
                              >
                                <div className="flex justify-between items-start">
                                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isToday ? 'bg-sky-500 text-white' : 'bg-slate-200 text-slate-600'
                                    }`}>
                                    {sch.day} {isToday ? '(Hari Ini)' : ''}
                                  </span>
                                  <span className="font-mono text-[10px] text-slate-500 font-bold">{sch.start_time} - {sch.end_time}</span>
                                </div>
                                <h4 className="font-bold text-slate-800 mt-1.5">{sch.course_name}</h4>
                                <div className="flex justify-between mt-1 text-[10px] text-slate-400">
                                  <span>{sch.room}</span>
                                  <span className="italic">{sch.lecturer_name}</span>
                                </div>
                              </div>
                            );
                          })}
              ...
              {activeTab === 'mata_kuliah' && (
                <div id="tab-dashboard-courses" className="space-y-6 animate-fade-in">

                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                    <div>
                      <h3 className="text-lg font-bold text-slate-800">Daftar Rencana Studi & Pengampu</h3>
                      <p className="text-xs text-slate-400 mt-1">Daftar mata kuliah semester sedang Anda tempuh di Esa Unggul.</p>
                    </div>
                  </div>
                        ...
                        >
                          <div>
                            <div className="flex justify-between items-start mb-3">
                              <span className="text-[10px] font-mono bg-sky-50 text-sky-700 px-2 py-0.5 rounded-lg border border-sky-100 font-extrabold">{crs.course_code}</span>
                              <span className="text-[10px] bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-lg font-semibold">{crs.credits} SKS</span>
                            </div>
                            <h4 className="font-black text-slate-800 text-sm leading-snug">{crs.course_name}</h4>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100">
                              <div className="flex justify-between text-[11px] mb-1 text-slate-500">
                                ...
                                <span className={`font-mono font-bold ${crs.attendance_rate < 75 ? 'text-rose-500' : 'text-emerald-500'}`}>{crs.attendance_rate}%</span>
                              </div>
                    ...
                    <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl border border-indigo-200 p-6 shadow-sm relative">
                      ...
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="md:w-1/2 space-y-4">
                          <div>
                            <span className="text-[10px] font-mono bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-lg font-black border border-indigo-100">{selectedCourse.course.course_code}</span>
                            <h3 className="text-xl font-bold text-slate-800 mt-2">{selectedCourse.course.course_name}</h3>
                            <p className="text-xs text-slate-500 mt-1">Dosen Pengampu: <span className="font-semibold">{selectedCourse.course.lecturer_name}</span></p>
                          </div>

                          <div className="p-3 bg-white rounded-xl border border-slate-100 text-xs">
                            <span className="text-[11px] font-bold text-slate-600 block mb-2">Kontrak Perkuliahan & Silabus</span>
                            <p className="text-slate-500 leading-normal">
                              Mata kuliah berfokus pada integrasi arsitektural SOA dan pertukaran data REST JSON dan SOAP XML. Disuplai materi lab serta model review.
                            </p>
                          </div>

                          {/* Meeting History Logs */}
                          <div>
                            <span className="text-xs font-bold text-slate-700 block mb-2">Riwayat Pertemuan Terbuka (12 Sesi)</span>
                            <div className="grid grid-cols-4 gap-2">
                              {selectedCourse.attendance.map((att: any, idx: number) => (
                                <div key={att.id} className="p-2 border border-slate-100 rounded-lg text-center bg-white">
                                  <span className="text-[9px] text-slate-400 block font-mono">Sesi {idx + 1}</span>
                                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full inline-block mt-1 ${att.status === 'Hadir' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-500'
                                    }`}>{att.status}</span>
                                </div>
                              ))}
                            </div>
                        ...
                        <div className="md:w-1/2 space-y-4">
                          <div>
                            <span className="text-xs font-bold text-slate-600 block mb-2">Materi Perkuliahan (Downloadable)</span>
                            <div className="space-y-1.5 text-xs">
                              {selectedCourse.materials.map((m: any, idx: number) => (
                                <div key={idx} className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-100 hover:border-sky-300">
                                  <div className="flex items-center gap-2">
                                    <FileText className="w-4 h-4 text-slate-400 shrink-0" />
                                    <span className="font-medium text-slate-700">{m.title}</span>
                                  </div>
                                  <span className="text-[9px] text-indigo-600 font-bold px-2 py-0.5 bg-indigo-50 rounded-lg">PDF</span>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div>
                            <span className="text-xs font-bold text-slate-600 block mb-2">Tugas Kelas & Kuis</span>
                            <div className="space-y-1.5 text-xs">
                              {selectedCourse.tasks.map((t: any, idx: number) => (
                                <div key={idx} className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-100">
                                  ...
                                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-lg ${t.due === 'Selesai' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                                    }`}>{t.due}</span>
                                </div>
                              ))}
                            </div>
              ...
              {activeTab === 'absensi' && (
                <div id="tab-dashboard-absensi" className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">Laporan Absensi & Syarat Ujian</h3>
                    <p className="text-xs text-slate-400 mt-1">Batas minimum kehadiran mahasiwa di Universitas Esa Unggul adalah <span className="font-bold text-slate-700">75%</span> untuk mengikuti UAS.</p>
                  </div>

                  <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
                    {attendance.map((att: any) => {
                      return (
                        <div key={att.course_id} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-3">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-mono bg-slate-200 px-1.5 py-0.5 rounded-md font-bold">{att.course_code}</span>
                                <h4 className="font-extrabold text-slate-800 text-xs md:text-sm">{att.course_name}</h4>
                              </div>
                              <p className="text-[11px] text-slate-400 mt-1 italic">{att.lecturer_name}</p>
                            </div>

                            <div className="flex items-center gap-2 shrink-0">
                              <span className="text-xs text-slate-500 font-bold">Rasio:</span>
                              <span className={`text-sm font-black font-mono ${att.is_safe ? 'text-emerald-600' : 'text-rose-600'}`}>
                                {att.attendance_summary.presenceRate}% (Hadir {att.attendance_summary.hadir}/{att.attendance_summary.actualLogged} Sesi)
                              </span>
                              <span className={`text-[9px] font-bold uppercase py-0.5 px-2 rounded-full ${att.is_safe ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                                }`}>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="w-full bg-slate-200 h-2.5 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{ width: `${att.attendance_summary.presenceRate}%` }}
                              ></div>
                            </div>

                            {/* Meeting details status chip strip */}
                            <div className="flex flex-wrap gap-4 text-[10px] font-bold text-slate-500 pt-1">
                              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-emerald-500"></span> Hadir: {att.attendance_summary.hadir} Sesi</span>
                              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-sky-500"></span> Izin: {att.attendance_summary.izin} Sesi</span>
                              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-indigo-500"></span> Sakit: {att.attendance_summary.sakit} Sesi</span>
                              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-red-400"></span> Alpha: {att.attendance_summary.alpha} Sesi</span>
                            </div>
                          </div>
                        </div>
                      );
              ...
              {activeTab === 'nilai' && (
                <div id="tab-dashboard-grades" className="space-y-6">
                  ...
                  <div className="flex justify-between items-center flex-wrap gap-2">
                    <div>
                      <h3 className="text-lg font-bold text-slate-800">Transkrip Nilai Akademik Kumulatif</h3>
                      <p className="text-xs text-slate-400 mt-1">Status dan evaluasi perolehan nilai mata kuliah semester ini.</p>
                    </div>
                    <div className="p-3 bg-sky-50 rounded-xl border border-sky-100 shrink-0 text-center">
                      <span className="text-[10px] text-slate-400 font-mono block">IPK SEMESTER 4</span>
                      <span className="text-xl font-black text-sky-700 font-mono">{calculateGPAValue().toFixed(2)}</span>
                    </div>
                  </div>
                          ...
                          {grades.map((gr: any) => {
                            return (
                              <tr key={gr.id} className="hover:bg-slate-50/50">
                                <td className="p-4 font-mono text-slate-400">{gr.course_code}</td>
                                <td className="p-4 font-bold text-slate-800">{gr.course_name}</td>
                                <td className="p-4 text-center font-bold">{gr.credits}</td>
                                <td className="p-4 text-center font-mono">{gr.assignment_score}</td>
                                <td className="p-4 text-center font-mono">{gr.quiz_score}</td>
                                <td className="p-4 text-center font-mono">{gr.midterm_score}</td>
                                <td className="p-4 text-center font-mono">{gr.final_score}</td>
                                <td className="p-4 text-center font-mono font-bold text-slate-900">{gr.final_grade}</td>
                                <td className="p-4 text-center">
                                  <span className={`inline-block font-mono font-black border uppercase px-2 py-0.5 rounded-lg ${gr.letter_grade.startsWith('A')
                                    ? 'bg-emerald-50 text-emerald-800 border-emerald-200'
                                    : gr.letter_grade.startsWith('B')
                                      ? 'bg-blue-50 text-blue-800 border-blue-200'
                                      : 'bg-amber-50 text-amber-800 border-amber-200'
                                    }`}>{gr.letter_grade}</span>
                                </td>
                              </tr>
                            );
                          })}
              ...
              {activeTab === 'pembayaran' && (
                <div id="tab-dashboard-payments" className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">Tagihan & Kewajiban Finansial</h3>
                    <p className="text-xs text-slate-400 mt-1">Kelola dan lunasi administrasi BPP, SKS, Praktikum, atau tagihan UTS/UAS Anda.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {payments.map((pay: any) => {
                      ...
                      return (
                        <div key={pay.id} className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start mb-3">
                              <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-slate-400">{pay.invoice_code}</span>
                              <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${isPaid
                            ...
                            <span className="text-xs text-slate-400 font-bold uppercase">{pay.payment_type}</span>
                            <h4 className="text-2xl font-black text-slate-800 mt-1">Rp {pay.amount.toLocaleString('id-ID')}</h4>

                            <div className="mt-4 space-y-1.5 text-xs text-slate-500">
                              <p className="flex justify-between"><span>Jatuh Tempo:</span> <span className="font-semibold text-slate-700">{pay.due_date}</span></p>
                              {pay.paid_at && <p className="flex justify-between"><span>Tanggal Bayar:</span> <span className="font-semibold text-slate-700">{new Date(pay.paid_at).toLocaleString('id-ID')}</span></p>}
                            </div>
                          </div>
                          ...
                          {isPending && (
                            <div className="mt-4 p-2.5 bg-amber-50 rounded-xl border border-amber-100 text-[11px] text-amber-800 leading-normal">
                              Bukti pembayaran Anda sedang diproses oleh Tim Admin Keuangan Universitas Esa Unggul. Notifikasi status akan dirilis segera.
                            </div>
                          )}
                        </div>
                      );
              ...
              {activeTab === 'notifikasi' && (
                <div id="tab-dashboard-notifications" className="space-y-6">
                  ...
                  <div className="flex justify-between items-center flex-wrap gap-2">
                    <div>
                      <h3 className="text-lg font-bold text-slate-800">Kotak Pemberitahuan Mahasiswa</h3>
                      <p className="text-xs text-slate-400 mt-1">Notifikasi akademik terbaru terkait perkuliahan dan tagihan.</p>
                    </div>
                    <button
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((not: any) => {
                        return (
                          <div
                            key={not.id}
                            onClick={() => !not.is_read && handleReadSingle(not.id)}
                            className={`p-4 transition-all leading-normal text-xs cursor-pointer ${not.is_read ? 'bg-white hover:bg-slate-50/50' : 'bg-sky-50/40 hover:bg-sky-50 border-l-4 border-sky-500 font-medium'
                              }`}
                          >
                            <div className="flex justify-between items-start mb-1 gap-4">
                              <h4 className={`text-sm ${not.is_read ? 'text-slate-700 font-bold' : 'text-slate-900 font-black'}`}>{not.title}</h4>
                              <span className="text-[10px] text-slate-400 font-mono tracking-tight shrink-0">{new Date(not.created_at).toLocaleDateString('id-ID')}</span>
                            </div>
                            <p className="text-slate-500 mt-1 leading-relaxed">{not.message}</p>
              ...
              {activeTab === 'profil' && (
                <div id="tab-dashboard-profile" className="space-y-6">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">Pengaturan Akun & Profil</h3>
                    <p className="text-xs text-slate-400 mt-1">Perbarui kata sandi, lengkapi alamat rumah, dan ubah pengaturan keamanan 2FA akun SIAKAD.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-5 md:col-span-2">
                      <h4 className="text-sm font-bold text-slate-800 mb-4 pb-2 border-b border-slate-100 uppercase tracking-wide">Informasi Mahasiswa</h4>

                      <form onSubmit={handleSaveProfile} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div>
                            <label className="block font-bold text-slate-600 mb-1">Nama Mahasiswa</label>
                            <input
                              type="text"
                              required
                          ...
                          <div>
                            <label className="block font-bold text-slate-600 mb-1">Nomor Telepon Kontak</label>
                            <input
                              type="text"
                              required
                          ...
                          <div className="md:col-span-2">
                            <label className="block font-bold text-slate-600 mb-1">Alamat Terpadu</label>
                            <textarea
                              rows={2}
                          ...
                          <div className="md:col-span-2 p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between">
                            <div>
                              <span className="font-bold text-slate-700 block text-xs">Dua Faktor Autentikasi (2FA OTP)</span>
                              <span className="text-[10px] text-slate-400 mt-1 block">Wajibkan verifikasi OTP email Gmail untuk setiap login.</span>
                            </div>
                            <input
                              type="checkbox"
                              checked={profileForm.two_fa_enabled}
                              onChange={(e) => setProfileForm(prev => ({ ...prev, two_fa_enabled: e.target.checked }))}
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 p-5">
                      <h4 className="text-sm font-bold text-slate-800 mb-4 pb-2 border-b border-slate-100 uppercase tracking-wide">Kata Sandi Pengaman</h4>

                      <form onSubmit={handleChangePassword} className="space-y-4 text-xs">
                        <div>
                          <label className="block font-bold text-slate-600 mb-1">Kata Sandi Lama</label>
                          <input
                            type="password"
                            required
                        ...
                        <div>
                          <label className="block font-bold text-slate-600 mb-1">Kata Sandi Baru</label>
                          <input
                            type="password"
                            required
                        ...
                        <div>
                          <label className="block font-bold text-slate-600 mb-1">Konfirmasi Kata Sandi Baru</label>
                          <input
                            type="password"
                            required
                ...
                <div id="tab-dashboard-performance" className="space-y-6">
                  ...
                  <div className="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div>
                        <span className="text-[10px] uppercase font-mono font-bold text-amber-400 tracking-wider bg-slate-800/80 px-2.5 py-1 rounded-full border border-slate-700/60 font-sans">Developer Hub & Benchmark Console</span>
                        ...
                        <p className="text-xs text-slate-300 mt-1.5 leading-relaxed max-w-4xl font-display">
                          Selamat datang di Auditing & Benchmark Hub. Halaman khusus ini memfasilitasi pengujian integritas middleware WSO2 Enterprise Service Bus (Carbon ESB) dan simulasi komparatif performa latensi throughput REST vs SOAP secara langsung pada klaster server SIAKAD.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200 p-5 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-[10px] uppercase font-mono font-extrabold tracking-widest bg-slate-50 text-slate-800 border border-slate-200/60 px-2.5 py-1 rounded-full animate-none">Middleware Orchestrator</span>
                          <span className={`inline-flex items-center gap-1.5 text-xs font-semibold ${useWso2Gateway ? 'text-emerald-600' : 'text-amber-600'}`}>
                            <span className={`h-2 w-2 rounded-full ${useWso2Gateway ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500 animate-pulse'}`}></span>
                            <span className="font-mono text-[10px]">{useWso2Gateway ? 'WSO2 active' : 'Direct active'}</span>
                          </span>
                        </div>
                        <h4 className="text-sm font-extrabold text-slate-800 font-display mb-1.5">Metode Routing API Portal</h4>
                        <p className="text-xs text-slate-500 leading-relaxed mb-4">
                          Ubah jalur lalu lintas data portal utama untuk mempelajari pengaruh intermediari middleware WSO2 ESB terhadap response-time real-time.
                        </p>
                      </div>
                        ...
                        <label className="flex items-start gap-3 cursor-pointer p-2.5 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              addEsbLog(`System settings rewritten. WSO2 Carbon Intermediary set ${e.target.checked ? 'ENABLED' : 'DISABLED'}`);
                              triggerAlert('success', `Gateway WSO2 ESB ${e.target.checked ? 'Diaktifkan (Rerouting)' : 'Dinonaktifkan (Direct API)'}`);
                            }}
                            className="w-4 h-4 text-sky-600 rounded border-slate-300 focus:ring-sky-500 mt-0.5"
                          />
                          <div>
                            <span className="text-xs font-bold text-slate-700 block select-none font-display">Hubungkan via WSO2 ESB Gateway</span>
                            <span className="text-[10px] text-slate-500 block mt-0.5 leading-normal">
                              {useWso2Gateway ? 'Proxy Port 3000 [/wso2/*] dengan header X-Gateway.' : 'Request langsung [/api/*] bypass ESB.'}
                            </span>
                          </div>
                        </label>
                      </div>
                    ...
                    <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200 p-5 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-[10px] uppercase font-mono font-extrabold tracking-widest bg-emerald-50 text-emerald-800 border border-emerald-100 px-2.5 py-1 rounded-full animate-none">REST JSON Protocol</span>
                          <span className="text-xs font-mono text-slate-400">Direct Endpoint /api</span>
                        </div>
                        <h4 className="text-sm font-extrabold text-slate-800 font-display mb-1.5">Operation: GET /api/courses</h4>
                        <p className="text-xs text-slate-500 leading-relaxed mb-4">
                          Menghasilkan daftar rencana studi aktif dengan payload JSON teringkas. Hemat bandwidth karena tidak memerlukan tag dokumen berulang.
                        </p>
                      </div>
                      ...
                      <div className="space-y-4 pt-4 border-t border-slate-100">
                        {benchResults.rest ? (
                          <div className="grid grid-cols-2 gap-2 text-center text-xs">
                            <div className="p-2 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[9px] text-slate-400 font-mono block uppercase">SPEED</span>
                              <span className="text-sm font-extrabold text-emerald-600 font-mono">{benchResults.rest.time} ms</span>
                            </div>
                            <div className="p-2 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[9px] text-slate-400 font-mono block uppercase">SIZE</span>
                              <span className="text-sm font-extrabold text-indigo-600 font-mono">{benchResults.rest.size} Bytes</span>
                            </div>
                          </div>
                        ) : (
                        ...
                        >
                          ...
                          <span className="font-display">Jalankan Audit REST API</span>
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="lg:col-span-1 bg-white rounded-2xl border border-slate-200 p-5 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-center mb-3">
                          <span className="text-[10px] uppercase font-mono font-extrabold tracking-widest bg-amber-50 text-amber-800 border border-amber-100 px-2.5 py-1 rounded-full animate-none">SOAP XML Protocol</span>
                          <span className="text-xs font-mono text-slate-400">WebService /soap</span>
                        </div>
                        <h4 className="text-sm font-extrabold text-slate-800 font-display mb-1.5">Operation: GetStudentCourses(nim)</h4>
                        <p className="text-xs text-slate-500 leading-relaxed mb-4">
                          Menghasilkan daftar rencana studi aktif terbungkus SOAP Envelope tervisualisasi via WSDL Schema Contract yang ketat.
                        </p>
                      </div>
                      ...
                      <div className="space-y-4 pt-4 border-t border-slate-100">
                        {benchResults.soap ? (
                          <div className="grid grid-cols-2 gap-2 text-center text-xs font-mono">
                            <div className="p-2 bg-slate-50 rounded-xl border border-slate-100">
                              ...
                              <span className="text-sm font-extrabold text-amber-600">{benchResults.soap.time} ms</span>
                            </div>
                            <div className="p-2 bg-slate-50 rounded-xl border border-slate-100">
                              <span className="text-[9px] text-slate-400 block uppercase font-sans">SIZE</span>
                              <span className="text-sm font-extrabold text-indigo-600">{benchResults.soap.size} Bytes</span>
                            </div>
                          </div>
                        ) : (
                          <div className="p-3 text-center text-[11px] text-slate-400 bg-slate-50 rounded-xl border border-slate-100 font-mono">
                        ...
                        >
                          <Play className="w-3.5 h-3.5 fill-current" />
                          <span className="font-display">Jalankan Audit SOAP XML</span>
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-950 p-5 rounded-2xl border border-slate-900 shadow-inner">
                    ...
                    <div className="font-mono text-xs text-slate-300 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                          <span className="font-extrabold text-amber-400 flex items-center gap-2 font-display">
                            ...
                            <span>Simulasi Server Gmail (Penerima OTP 2FA)</span>
                          </span>
                          <span className="text-[9px] text-slate-500 font-sans">Live Listening...</span>
                        </div>
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                          {gmailInbox.length === 0 ? (
                            <div className="text-slate-500 py-8 text-center leading-normal text-[11px] font-sans">
                              ...
                              Gmail simulator ini aktif saat verifikasi login, penambahan Rencana Kuliah, pembayaran, dll.
                            </div>
                          ) : (
                            gmailInbox.map((mail: any) => {
                              ...
                              return (
                                <div key={mail.id} className="p-2.5 bg-slate-900/60 rounded border border-slate-800 flex items-start gap-2.5 text-[11px]">
                                  <div className="bg-slate-800 p-1.5 rounded shrink-0 leading-none">
                                    <ShieldCheck className="w-4.5 h-4.5 text-indigo-400" />
                                  </div>
                                  <div className="grow leading-normal">
                                    <div className="flex justify-between text-[10px]">
                                      <span className="text-indigo-300 font-bold">To: {mail.email || 'mahasiswa@esaunggul.ac.id'}</span>
                                      <span className="text-slate-500">{new Date(mail.created_at).toLocaleTimeString()}</span>
                                    </div>
                                    <h4 className="text-slate-200 font-bold mt-0.5 text-[11px] font-display">{mail.title}</h4>
                                    <p className="text-slate-400 text-[10px] mt-0.5 italic">{mail.message}</p>
                                      ...
                                      <div className="mt-2 flex items-center gap-2">
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            ...
                                            triggerAlert('success', `OTP ${codeStr} disalin!`);
                                            addEsbLog(`Developer: Copied OTP ${codeStr} client-side.`);
                                          }}
                      ...
                      <div className="text-slate-500 text-[9px] pt-2 border-t border-slate-900/60 mt-3 leading-normal font-sans">
                        Mekanisme bypass SMTP: Karena port mail di-sandboxed default pada Cloud Run, dashboard ini memunculkan salinan surel yang aman demi kelancaran demo.
                      </div>
                    </div>
                    ...
                    <div className="font-mono text-xs text-slate-300 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                          ...
                          <span className="text-[9px] bg-slate-850 text-indigo-400 px-1.5 py-0.5 rounded font-sans">Carbon v5.3.0</span>
                        </div>
                        ...
                        <div className="space-y-1 max-h-48 overflow-y-auto pr-1 text-[10px] leading-relaxed">
                          {esbLogs.length === 0 ? (
                            <div className="text-slate-500 py-8 text-center text-[11px] font-sans">
                              Belum ada log middleware yang dihasilkan.<br />
                              Lakukan pengujian benchmark atau manipulasi data profil untuk memforward log WSO2 Carbon ESB.
                            </div>
                          ) : (
                            esbLogs.map((logStr, idx) => (
                              <div key={idx} className="hover:bg-slate-900 p-0.5 rounded truncate">
                      ...
                      <div className="text-slate-500 text-[9px] pt-2 border-t border-slate-900/60 mt-3 leading-normal font-sans">
                        Standard WSO2 Enterprise Header Injection:<br />
                        <span className="text-indigo-400">X-Gateway-Server: WSO2-Carbon-ESB-v5.3.0</span> | <span className="text-sky-400">X-Gateway-Processing-Ms</span>
                      </div>
                    </div>
                  ...
                  {performanceHistory.length > 0 && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
                      <div className="flex justify-between items-center flex-wrap gap-2">
                        <h3 className="font-extrabold text-slate-800 text-sm">Hasil Riwayat Grafik Pengujian Komparatif</h3>
                        <button
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                        ...
                        <div className="p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                          <span className="font-bold text-slate-700 block mb-3 text-center">Kecepatan Response Time (ms) - Lebih Rendah Lebih Baik</span>
                          <div className="h-52 w-full">
                            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                              <BarChart data={performanceHistory.slice(-10)}>
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="time" name="Time (ms)" fill="#38bdf8">
                                  {performanceHistory.slice(-10).map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={entry.api_type === 'REST' ? '#10b981' : '#f59e0b'} />
                                  ))}
                                </Bar>
                              </BarChart>
                            </ResponsiveContainer>
                        ...
                        {/* Payload size chart */}
                        <div className="p-4 rounded-xl border border-slate-100 bg-slate-50/50">
                          <span className="font-bold text-slate-700 block mb-3 text-center">Ukuran Payload Transaksi (Bytes) - Lebih Rendah Lebih Efisien</span>
                          <div className="h-52 w-full">
                            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                              <BarChart data={performanceHistory.slice(-10)}>
                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="size" name="Payload (Bytes)" fill="#6366f1">
                                  {performanceHistory.slice(-10).map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={entry.api_type === 'REST' ? '#10b981' : '#f59e0b'} />
                                  ))}
                                </Bar>
                      ...
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 text-xs text-slate-600 leading-relaxed whitespace-pre-line">
                        <span className="font-bold text-slate-800 block mb-1.5 uppercase font-mono text-[10px] tracking-wider">Kesimpulan Otomatis Sistem Benchmark:</span>
                        {getAutoConclusion()}
                      </div>
                    </div>
                  )}
                  ...
                  {(benchResults.rest || benchResults.soap) && (
                    <div className="bg-slate-900 text-slate-100 rounded-2xl p-5 shadow-xs font-mono text-xs">
                      <span className="inline-block text-[10px] bg-slate-800 uppercase font-bold tracking-widest text-slate-400 py-0.5 px-2 rounded mb-3">Payload Inspector Console</span>

                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">

                        {benchResults.rest && (
                          <div className="space-y-2">
                            <span className="text-emerald-400 block text-xs">REST JSON Response:</span>
      ...
      {showDevPanel && (
        ...
        >
          ...
          <div className="bg-slate-900 px-4 py-2.5 flex items-center justify-between border-b border-slate-800 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={isDevStreamExpanded ? "Minimalkan Panel" : "Maksimalkan Panel"}
              >
                {isDevStreamExpanded ? (
            ...
            <div className="flex flex-col max-h-96 min-h-64 divide-y divide-slate-900 bg-slate-950">
              ...
              <div className="p-3.5 flex flex-col min-h-[140px] max-h-48 overflow-hidden">
                <div className="flex items-center justify-between mb-2 pb-1 border-b border-slate-900">
                  ...
                  <span className="text-[8px] text-slate-500 font-sans">Server listening...</span>
                </div>
                ...
                <div className="grow overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                  {gmailInbox.length === 0 ? (
                    ...
                    gmailInbox.map((mail: any) => {
                      ...
                      return (
                        <div key={mail.id} className="p-2 bg-slate-900 border border-slate-800/60 rounded flex items-start gap-2.5 text-[10px]">
                          <div className="bg-slate-850 p-1 rounded shrink-0 mt-0.5">
                            <ShieldCheck className="w-3.5 h-3.5 text-indigo-400" />
                          </div>
                          <div className="grow leading-relaxed">
                            <div className="flex justify-between text-[8px] text-slate-500">
                              <span>To: {mail.email || 'mahasiswa@esaunggul.ac.id'}</span>
                              <span>{new Date(mail.created_at).toLocaleTimeString()}</span>
                            </div>
                            <h5 className="text-slate-200 font-bold font-sans mt-0.5 text-[10px]">{mail.title}</h5>
                            <p className="text-slate-400 italic text-[9px] mt-0.5">{mail.message}</p>
                              ...
                              <div className="mt-1.5 flex items-center gap-1.5">
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    navigator.clipboard.writeText(codeStr);
                                    setOtpForm(codeStr);
                                    triggerAlert('success', `OTP ${codeStr} disalin & diisi!`);
                                    addEsbLog(`Developer: Copied OTP ${codeStr} client-side.`);
                                  }}
              ...
              <div className="p-3.5 flex flex-col min-h-[140px] max-h-48 overflow-hidden">
                <div className="flex items-center justify-between mb-2 pb-1 border-b border-slate-900">
                  ...
                  <span className="text-[8px] text-slate-500 font-sans">Active Router Logs</span>
                </div>
          ...
          <div className="bg-slate-900 px-3.5 py-1.5 border-t border-slate-800 text-[8px] text-slate-500 flex justify-between shrink-0 font-sans">
            ...
            <span className="text-amber-500 font-mono">X-WSO2-Header Injection Active</span>
          </div>
        </div>
      )}

      {/* Bottom Footer Credits branding (Official & Professional Layout) */}
      <footer id="siakad-footer-panel" className="bg-slate-100 border-t border-slate-200 text-slate-500 text-xs py-6 shrink-0 font-display">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="font-bold text-slate-700">Sistem Informasi Akademik (SIAKAD) Online</p>
            <p className="text-[10px] text-slate-400 mt-1">Portal Integrasi & Benchmark Middleware Layanan Keimigrasian & Rencana Studi Mahasiswa</p>
          </div>
